    ...
    ${
      ...
      {activeTool && (
        <motion.div
          ...
          className="max-w-2xl mb-10 bg-white/20 backdrop-blur-3xl p-6 rounded-2xl shadow-xl"
        >
        ...
        {keysLayout.map((row, rowIndex) => (
          <div key={rowIndex} className="flex justify-center space-x-3">
            {row.map((key, keyIndex) => {
              const refIndex = rowIndex * 10 + keyIndex; // stable ref index
              return (
                <div
                  key={keyIndex}
                  ref={(el) => (keyRefs.current[refIndex] = el)}
                  onClick={() => handleKeyClick(key)}
                  className="relative w-12 h-12 md:w-14 md:h-14 flex items-center justify-center rounded-xl cursor-pointer font-bold text-lg select-none
