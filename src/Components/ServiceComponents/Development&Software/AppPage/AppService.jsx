...
const AppService = () => {
  ...
  return (
    <section className="bg-neutral-950 py-16 sm:py-20 md:py-24 px-4 sm:px-6 md:px-12 lg:px-20 bg-fixed">
      {/* Title + Description */}
      <motion.div
        initial={{ opacity: 0, y: 40 }}
        whileInView={{ opacity: 1, y: 0 }}
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
        {services.map((title, i) => (
          <motion.div
            ...
            viewport={{ once: false, amount: 0.3 }}
            whileHover={{ scale: 1.05 }}
            className="bg-neutral-900 p-6 sm:p-8 rounded-2xl shadow-lg border border-neutral-800 hover:border-white transition"
          >
            <h3 className="font-bold mb-2 text-lg sm:text-xl text-white">
              {title}
            </h3>
