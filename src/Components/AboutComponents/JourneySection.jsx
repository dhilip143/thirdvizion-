...
export default function JourneySection() {
  ...
  useEffect(() => {
    ...
    // Animate milestones one by one, alternating up/down
    sections.forEach((el, index) => {
      const direction = index % 2 === 0 ? -50 : 50; // even: up, odd: down
      gsap.fromTo(
        el,
  ...
  return (
    <section ref={containerRef} className="relative w-full bg-black overflow-hidden">
      ...
      {/* Wavy SVG Path */}
      <svg className="absolute top-32 left-0 z-10" width="4000" height="120" viewBox="0 0 4000 120" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          ref={pathRef}
          d="M0 60 C 500 0, 1000 120, 1500 60 S 2500 60, 3000 60 S 3500 60, 4000 60"
          stroke="url(#gradLine)"
          strokeWidth="4"
          fill="none"
        />
      ...
      {/* Scroll Wrapper */}
      <div ref={scrollWrapperRef} className="flex items-center space-x-40 px-20 py-56 absolute top-0 left-0">
        {milestones.map((milestone, index) => (
          <div
            key={index}
            ref={(el) => (itemsRef.current[index] = el)}
          ...
          >
            ...
            {/* Content */}
            <h3 className="mt-6 text-2xl md:text-3xl font-bold text-white">{milestone.title}</h3>
            <span className="text-sm mt-1 text-orange-400">{milestone.date}</span>
            <p className="mt-4 text-gray-300 leading-relaxed">{milestone.description}</p>
          </div>
        ))}
      </div>
    </section>
